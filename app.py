...
def upload_image():
    ...
    if request.method == "POST":
        ...
        # Wait a moment and try to get caption from database (Lambda might have processed it)
        import time
        caption = "Caption is being generated by Lambda function..."
        
        # Try to get caption from database after a short delay
            ...
            try:
                ...
                if connection:
                    ...
                    if result and result[0]:
                        caption = result[0]
                        print(f"Found caption for {filename}: {caption[:50]}...")
                        break
                    else:
                        print(f"No caption found yet for {filename}, attempt {attempt + 1}")
            except Exception as e:
                print(f"Error checking for caption: {e}")
        ...
        return render_template(
            ...
            caption=caption,
            filename=filename,
            show_refresh_button=caption.startswith("Caption is being generated")
        )
...
def get_caption(filename):
    """
    Fetch the latest caption for a specific image from the database.
    """
    try:
        connection = get_db_connection()
        if connection is None:
        ...
        if result and result[0]:
            ...
            try:
                ...
                return render_template("upload.html", error=f"Error retrieving image: {str(e)}")
        else:
            return render_template("upload.html", error="Caption not found or still being generated")
